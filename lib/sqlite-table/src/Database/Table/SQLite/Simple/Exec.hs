...
-- | Retrieve those rows from the table that satisfy a condition.
--
...
--
-- Uses a prepared SQL statement internally for efficiency.
--
-- SQL: @INSERT INTO tablename VALUES ...@, repeated.
...
--
-- SQL: @UPDATE tablename SET ... WHERE condition@.
updateWhere
    :: IsTableSql t
    => Expr.Expr Bool
    -- ^ Condition.
    -> [Stmt.Update]
    -- ^ 'Update's to be performed.
    -> proxy t
    -- ^ Table type.
    -> SqlM ()
