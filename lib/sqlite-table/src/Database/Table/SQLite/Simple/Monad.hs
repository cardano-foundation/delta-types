...
Will throw an exception if the connection cannot be opened,
...
(See also 'Sqlite.open' in "Database.SQLite.Simple".)
-}
...
--
-- (See also 'Sqlite.close' in "Database.SQLite.Simple".)
close :: Connection -> IO ()
...
--
-- (See also 'Sqlite.withConnection' in "Database.SQLite.Simple".)
...
* mutable state — The state of the database is represented as mutable state.
* exceptions

When using 'runSqlM', an action from this monad is executed atomically:
Either the whole action succeeds, then all state changes persists,
or the action throws an exception, then none of state changes persist.
-}
