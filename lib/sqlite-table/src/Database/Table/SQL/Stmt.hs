...
createTable proxy =
    CreateTable
        (getTableName proxy)
        (zip (getColumnNames proxy) (getColumnSqlTypes proxy))

-- | Select all rows from the table.
selectAll :: IsTableSql t => proxy t -> Stmt
selectAll proxy =
    Select (getColumnNames proxy) (getTableName proxy) All
...
selectWhere expr proxy =
    Select (getColumnNames proxy) (getTableName proxy) (Where expr)

-- | Insert one row into the corresponding table.
insertOne :: IsTableSql t => proxy t -> Stmt
insertOne proxy =
    Insert (getTableName proxy) (getColumnNames proxy)

-- | Representation of a column update,
-- for use with 'Database.Table.SQLite.Simple.updateWhere'.
--
-- Use '(=.)' to construct an 'Update'.
data Update = Update
    { lhs :: ColumnName
    , rhs :: Var.Value
    }
